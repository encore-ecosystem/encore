...
typedef void (*encore_heap_destroy_fn)(void *);

typedef struct {
    void *ptr;
    encore_heap_destroy_fn destroy;
} encore_heap_destroy_job;

#define ENCORE_DESTROY_INLINE_JOBS 64

typedef struct encore_heap_destroy_context {
    encore_heap_destroy_job *jobs;
    size_t len;
    size_t cap;
    encore_heap_destroy_job inline_jobs[ENCORE_DESTROY_INLINE_JOBS];
} encore_heap_destroy_context;

static _Thread_local encore_heap_destroy_context *encore_active_destroy_context;

static bool encore_heap_queue_destroy(encore_heap_destroy_context *context,
                                      void *ptr,
                                      encore_heap_destroy_fn destroy) {
    if (context->len == context->cap) {
        if (context->cap > SIZE_MAX / 2 ||
            context->cap * 2 > SIZE_MAX / sizeof(encore_heap_destroy_job)) return false;
        size_t next_cap = context->cap * 2;
        encore_heap_destroy_job *next;
        if (context->jobs == context->inline_jobs) {
            next = malloc(next_cap * sizeof(encore_heap_destroy_job));
            if (next != NULL) {
                memcpy(next, context->inline_jobs,
                       context->len * sizeof(encore_heap_destroy_job));
            }
        } else {
            next = realloc(context->jobs, next_cap * sizeof(encore_heap_destroy_job));
        }
        if (next == NULL) return false;
        context->jobs = next;
        context->cap = next_cap;
    }
    context->jobs[context->len++] = (encore_heap_destroy_job){ptr, destroy};
    return true;
}

static void encore_heap_drain_destroy_jobs(encore_heap_destroy_context *context) {
    while (context->len > 0) {
        encore_heap_destroy_job job = context->jobs[--context->len];
        job.destroy(job.ptr);
    }
}
...
#endif
void encore_heap_release_deferred(void *ptr, encore_heap_destroy_fn destroy) {
    if (ptr == NULL || destroy == NULL) return;
    encore_heap_block *block = ((encore_heap_block *)ptr) - 1;
    if (atomic_fetch_sub_explicit(&block->meta.refs, 1, memory_order_acq_rel) != 1) return;

    encore_heap_destroy_context *active = encore_active_destroy_context;
    if (active != NULL) {
        if (encore_heap_queue_destroy(active, ptr, destroy)) return;
        /* Allocation failure preserves correctness, with recursive destruction
           as the same fallback used before deferred destruction was added. */
        destroy(ptr);
        return;
    }

    encore_heap_destroy_context context;
    context.jobs = context.inline_jobs;
    context.len = 0;
    context.cap = ENCORE_DESTROY_INLINE_JOBS;
    encore_active_destroy_context = &context;
    destroy(ptr);
    encore_heap_drain_destroy_jobs(&context);
    encore_active_destroy_context = NULL;
    if (context.jobs != context.inline_jobs) free(context.jobs);
}
