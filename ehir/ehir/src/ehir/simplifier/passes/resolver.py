...
class ResolverPass(SimplifierPass):
    ...
    def _build_type_name_aliases(self) -> None:
        ...
        for name in [*self.structs, *self.enums]:
            add(name, name)
            short = name.split("::")[-1]
            add(short, name)
            if "::lib::" in name:
                add(name.replace("::lib::", "::"), name)

        self._type_name_aliases = {alias: canonical for alias, canonical in aliases.items() if canonical is not None}
    ...
    def _validate_impl_coherence(self) -> None:
        ...
        for impl in self.impls:
            ...
            if trait_decl is None:
                ...
                trait_decl = next(
                    (trait for trait in self.traits.values() if trait.name.split("::")[-1] == short), None
                )
            if trait_decl is None:
                raise EhirCompileError(
    ...
    def _resolve_callable_signature(
        ...
        def _direct_self_type(fn_directive, name: str) -> Type | None:
            params = getattr(fn_directive, "params", [])
            if not params or params[0].name != "self":
                return None
            return _owner_self_type_from_name(name)
                ...
                if len(explicit_generics) == len(fn_generics):
                    ...
                    if inferred is not None:
                        for generic_param in fn_generics:
                            ...
                            if explicit is None:
                                continue
                            explicit_is_placeholder = not explicit.generics and explicit.name == generic_param.name
                ...
                for impl in self.impls:
                    ...
                    if instr.generics:
                        ...
                        if trait_arg_mismatch:
                            continue
                        resolved_trait_args = [
                            self._resolve_type(self._replace_type(arg, mapping, recv_base)) for arg in impl.trait_args
                        ]
                        if any(
        ...
        if trait_decl is not None:
            ...
            if trait_method is not None:
                ...
                ret_type = (
                    self._replace_type(trait_method.ret_type, {}, recv_base)
                    if recv_base is not None
                    else trait_method.ret_type
                )
        ...
        for impl in self.impls:
            ...
            if impl.for_type.name != owner_base.name and impl_short != owner_short:
                continue
            method = next(
                (m for m in impl.methods if m.name == method_name or m.name.startswith(f"{method_name}__")),
                None,
            )
            if method is None:
                continue
    ...
    def _types_compatible(self, lhs: Type, rhs: Type) -> bool:
        key = (
            frozenset(self._current_generic_names),
            self._type_cache_key(lhs),
            self._type_cache_key(rhs),
        )
        cached = self._types_compatible_cache.get(key)
        if cached is not None:
            return cached
        result = self._types_compatible_uncached(lhs, rhs)
        self._types_compatible_cache[key] = result
        return result

    def _types_compatible_uncached(self, lhs: Type, rhs: Type) -> bool:
    ...
    def _contains_unbound_generic(self, typ: Type) -> bool:
        ...
        if isinstance(typ, Reference):
            return self._contains_unbound_generic(typ.pointee)
        is_symbolic = typ.name not in self.structs and typ.name not in self.enums and typ.name[:1].isupper()
        if is_symbolic:
            return True
    ...
    def _resolve_type(self, typ: Type) -> Type:
        key = (frozenset(self._current_generic_names), self._type_cache_key(typ))
        cached = self._resolve_type_cache.get(key)
        if cached is not None:
            return cached
        resolved = self._resolve_type_uncached(typ)
        self._resolve_type_cache[key] = resolved
        return resolved

    def _resolve_type_uncached(self, typ: Type) -> Type:
        if isinstance(typ, Pointer):
        ...
        return Type(self._type_name_aliases.get(typ.name, typ.name), [self._resolve_type(g) for g in typ.generics])

    def _type_cache_key(self, typ: Type) -> tuple:
        if isinstance(typ, Pointer):
            return ("ptr", self._type_cache_key(typ.pointee))
        if isinstance(typ, Reference):
            return ("ref", self._type_cache_key(typ.pointee))
        return (type(typ).__name__, typ.name, tuple(self._type_cache_key(generic) for generic in typ.generics))
